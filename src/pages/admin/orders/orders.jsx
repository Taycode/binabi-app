...
const NoOrderInView = () => {
  return (
    <div className="no-order">
      <span className="material-icons icon">shopping_basket</span>
      <h1>No Selected Order</h1>
    </div>
  );
};
...
const UpdateOrderStatusButton = ({
  ...
  const handleUpdateOrder = () => {
    ...
    setUpdateSuccess(false);
    order
      .updateOrder(currentOrder.orderId, { status })
      .then(() => {
  ...
  return (
    <>
      <button
        className={`update-order-status ${status} ${updating && "updating"}`}
        onClick={() => handleUpdateOrder({ status })}
      >
        {children}
      </button>
    ...
    <section className="order-in-view">
      <section className="orders-view-content-box">
        <div className="exit-action-container" onClick={backAction}>
          ...
          <p className="exit-action-container--action-name">Back</p>
        </div>
        <div className="customer-card">
          <h3 className="customer-name">{currentOrder.name}</h3>
          <p className="customer-address">{currentOrder.address}</p>
        ...
        <div className="order-details">
          ...
          <div className="order-details-content">
            <OrderDetail label="Customer Name">{currentOrder.name}</OrderDetail>
  ...
  return (
    <div className="order-header">
      <div className="sorting-field">
        ...
        <select
          className="sorting-field--select-box"
          onChange={(e) => onSelectSortMethod(e.target.value)}
        >
          <option
            value={JSON.stringify({ field: "timeCreated", order: "desc" })}
          >
            {" "}
            Default{" "}
          </option>
          ...
          <option
            value={JSON.stringify({ field: "timeCreated", order: "asc" })}
          >
            {" "}
            Time (a-z){" "}
          </option>
          <option value={JSON.stringify({ field: "name", order: "asc" })}>
            {" "}
            Name (a-z){" "}
          </option>
          <option value={JSON.stringify({ field: "name", order: "desc" })}>
            {" "}
            Name (z-a){" "}
          </option>
          <option value={JSON.stringify({ field: "capacity", order: "desc" })}>
            {" "}
            No. of Kg (z-a){" "}
          </option>
          <option value={JSON.stringify({ field: "capacity", order: "asc" })}>
            {" "}
            No. of Kg (a-z){" "}
          </option>
          <option value={JSON.stringify({ field: "status", order: "asc" })}>
            {" "}
            Status (a-z){" "}
          </option>
          <option value={JSON.stringify({ field: "status", order: "desc" })}>
            {" "}
            Status (z-a){" "}
          </option>
        </select>
...
const FetchMoreOrdersButton = ({ onFetchOrders }) => {
  return (
    <button className="orders-load-button" onClick={onFetchOrders}>
      Load More &nbsp; &#129171;
    </button>
  );
};
...
export const AdminPanelOrders = () => {
  ...
  const [newLiveOrder, setNewLiveOrder] = useState({})
  ...
  useEffect (() => {
    if (!orders.find(order => order.orderId === newLiveOrder.orderId)) {
      setOrders([newLiveOrder, ...orders])
    }
  }, [newLiveOrder.orderId])
  
  function updateOrdersAfterSnapshot (querySnapshot) {
    const _finalOrdersList = []
    querySnapshot.forEach((_doc) => {
      _finalOrdersList.push({ orderId: _doc.id, ..._doc.data() })
    })
    setNewLiveOrder(_finalOrdersList[0])
  }
  ...
  useEffect(() => {
    ...
    const limit = 5;
    const lastDoc = orders.length > 0 && orders[orders.length - 1][sortOptions.field];
    const orderRef = db.collection("orders");
    const ordersList = orderRef.orderBy(sortOptions.field, sortOptions.order).limit(limit)
    ...
    .then((data) => {
      if (data.length < 1) {
        ...
        ordersList.onSnapshot(updateOrdersAfterSnapshot)
      })
  }, [sortOptions]);
  ...
  function handleLoadMore() {
    ...
    order
      .getOrders(
        sortOptions.field,
        sortOptions.order,
        orders.length > 0 && orders[orders.length - 1][sortOptions.field]
      )
      .then((data) => {
        if (data.length < 1) {
      ...
      <section className="page-container">
        ...
        {Object.keys(orderInView).length > 0 && (
          <div className="order-view-overlay">
            <div
              className={`orders-view ${orderInView.orderId ? "in-view" : ""}`}
            >
              <OrderView
                order={orderInView}
                isActive
                backAction={backActionForOrderInView}
                onUpdateOrder={(uOrder) => popInUpdatedOrder(uOrder)}
              />
            </div>
          </div>
        )}
